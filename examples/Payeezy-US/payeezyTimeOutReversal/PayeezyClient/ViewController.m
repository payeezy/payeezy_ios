...
#define KToken       @"fdoa-10b9547c3dfd3d5d3d9e01c726b952fb10b9547c3dfd3d5d"

#define Exchange_rate_URL  @"https://api-qa.payeezy.com/v1/transactions/exchange_rate"

/* use following url for Primary transactions */
#define TransactionsURL     @"https://api-qa.payeezy.com/v1/transactions"
 ...
 * \returns
 */
- (IBAction)reverseTransaction:(NSString *)merchant_ref  :(NSString *)trans_type  :(NSString *)totalamount  :(NSString *)reversalId {
    ...
    NSLog(@"AuthorizeReversalTransaction: merchant_ref: %@",merchant_ref);
  
    NSLog(@"AuthorizeReversalTransaction: trans_type: %@",trans_type);
   
    NSLog(@"AuthorizeReversalTransaction: totalamount: %@",totalamount);
    NSLog(@"AuthorizeReversalTransaction: reversalId: %@",reversalId);
    
     NSLog(@"AuthorizeReversalTransaction: KApiKey: %@",KApiKey);
     NSLog(@"AuthorizeReversalTransaction: KApiSecret: %@",KApiSecret);
     NSLog(@"AuthorizeReversalTransaction: KToken: %@",KToken);
     NSLog(@"AuthorizeReversalTransaction: TransactionsURL: %@",TransactionsURL);
    
    [myClient submitReversalTransactionWithCreditCardDetails:void_transaction[@"merchant_ref"] transactionType:void_transaction[@"transaction_type"]  paymentMethodType:void_transaction[@"method"] totalAmount:void_transaction[@"amount"] currencyCode:void_transaction[@"currency_code"] reversalId:void_transaction[@"reversal_id"] completion:^(NSDictionary *dict, NSError *error) {
...
- (IBAction)AuthorizeReversalTransaction:(id)sender {
    ...
    NSLog(@"AuthorizeReversalTransaction: uuid: %@",uuid);
    ...
    [myClient submitAuthorizePurchaseTransactionWithCreditCardDetails:credit_card[@"cvv"] cardExpMMYY:credit_card[@"exp_date"] cardNumber:credit_card[@"card_number"] cardHolderName:credit_card[@"cardholder_name"]  cardType:credit_card[@"type"] currencyCode:@"USD" totalAmount:@"200" merchantRef:@"abc1412096293369"   transactionType:@"authorize" token_type:@"FDToken" method:@"token" reversalId:uuid completion:^(NSDictionary *dict, NSError *error)
     {
         ...
         if (error == nil)
         {
             ...
             if ([[dict objectForKey:@"transaction_status"] isEqualToString:@"declined"]){
                 
            
                 
                 [self reverseTransaction:@"abc1412096293369"  :@"void"  :@"200" :uuid];
...
- (void)viewDidLoad {
    ...
    self.items = [[NSArray alloc] initWithObjects:@"Select Test Credit Card",@"Visa-4012000033330026-1218-123",@"Master-5595520004787484-0430-123",@"Amex-373953192351004-1018-1234",@"Discover-6011000990099818-1218-123",nil];
